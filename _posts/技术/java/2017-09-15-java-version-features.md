...
1. ZGC: Uncommit Unused Memory  ZGC 可以将未使用的堆内存返回给操作系统
1. Reimplement the Legacy Socket API 重新实现了 Socket 接口的逻辑。原来的老逻辑还存在。通过指定-Djdk.net.usePlainSocketImp 参数
1. **Switch Expressions (Preview) 增强了 Java12 引入的 Switch 表达式，可以使用yield关键字返回结果**
1. **Text Blocks (Preview)  文本框，避免对多行文本字符的转义。类似于 grovvy 中的三引号括起来的内容。** 
